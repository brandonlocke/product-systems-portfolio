...
import { cases, featuredCases, findCase } from "../cases";

export function generateStaticParams() { return cases.map(({ slug }) => ({ slug })); }

function EvidenceField({ slug, value, label, status }: { slug: string; value: string; label: string; status: string }) {
  return (
    <div className="evidence-field" aria-label={`${value} ${label}`}>
      <div className="evidence-copy"><p>Proof available now</p><strong>{value}</strong><span>{label}</span><small>{status}</small></div>
      <div className="evidence-artifact"><CaseArtifact slug={slug} /></div>
    </div>
  );
}
...
export default async function CasePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!item) notFound();
  const index = featuredCases.findIndex((entry) => entry.slug === slug);
  const nextIndex = (index + 1) % featuredCases.length;
  const nextCase = featuredCases[nextIndex];
  ...
  return (
    <main className={`case-page case-${index + 1}`}>
      <nav className="nav-shell case-nav" aria-label="Case study navigation">
        ...
        <div className="case-jump"><a href="#challenge">Context</a><a href="#role">Role</a><a href="#decisions">Decisions</a><a href="#outcome">Evidence</a></div>
      ...
      <EvidenceField slug={item.slug} value={item.proofValue} label={item.proofLabel} status={item.status} />

      <article className="case-body">
        <section className="case-section section-challenge" id="challenge">
          <p className="case-section-number">01 / Business context</p>
          <div><h2>{item.challengeHeading}</h2>{item.challenge.map((p) => <p key={p}>{p}</p>)}</div>
        </section>
        <section className="case-section section-role" id="role">
          <p className="case-section-number">02 / Strategic role</p>
          <div><h2>{item.roleHeading}</h2>{item.role.map((p) => <p key={p}>{p}</p>)}</div>
        </section>
        ...
        <section className="outcome-panel" id="outcome"><p className="case-section-number">04 / Evidence + learning</p><h2>{item.outcome}</h2><p>{item.outcomeDetail}</p></section>
      </article>
